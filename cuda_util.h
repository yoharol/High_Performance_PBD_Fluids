...
static void check_api_error_(cudaError_t e, const char* msg, const char* file,
                             int line) {
  if (e) {
...
#define check_launch_error(exp)                    \
  do {                                             \
    exp;                                           \
    check_launch_error_(#exp, __FILE__, __LINE__); \
  } while (0)
 ...
 */
__attribute__((unused)) static int get_freq() {
...
/**
   @brief wrap cudaMalloc.  cudaMalloc + error check + more ordinary malloc-like
   interface (return pointer)
 */
__attribute__((unused)) static void* dev_malloc(size_t sz) {
  void* a = 0;
...
/**
   @brief wrap cudaFree
 */
__attribute__((unused)) static void dev_free(void* a) { cudaFree(a); }

/**
   @brief wrap cudaMemcpy to copy from device to host (and check an error if
   any)
 */
__attribute__((unused)) static void to_host(void* dst, void* src, size_t sz) {
  check_api_error(cudaMemcpy(dst, src, sz, cudaMemcpyDeviceToHost));
}

/**
   @brief wrap cudaMemcpy to copy from host to device (and check an error if
   any)
 */
__attribute__((unused)) static void to_dev(void* dst, void* src, size_t sz) {
 ...
 */
__device__ inline int get_nthreads_x() { return gridDim.x * blockDim.x; }

/**
   @brief number of threads along y-dimension
 */
__device__ inline int get_nthreads_y() { return gridDim.y * blockDim.y; }

/**
   @brief number of threads along z-dimension
 */
__device__ inline int get_nthreads_z() { return gridDim.z * blockDim.z; }
